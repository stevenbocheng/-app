...
const TripInfoView: React.FC<TripInfoViewProps> = ({
  ...
  const handleOutboundChange = (field: keyof typeof flightInfo.outbound, value: string) => {
    onFlightChange({
      ...flightInfo,
      outbound: { ...flightInfo.outbound, [field]: value }
    });
  };
  ...
  const handleInboundChange = (field: keyof typeof flightInfo.inbound, value: string) => {
    onFlightChange({
      ...flightInfo,
      inbound: { ...flightInfo.inbound, [field]: value }
    });
  };

  const handleHotelUpdate = (field: keyof HotelInfo, value: string) => {
    onHotelChange({ ...hotelInfo, [field]: value });
  };
      ...
      <div className="bg-white rounded-[24px] p-5 shadow-sm border border-slate-100">
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">出發</label>
            <input 
              type="date" 
              value={startDate}
              onChange={(e) => onDateChange(e.target.value, endDate)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-blue-500"
            />
          </div>
          <div className="flex-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">回程</label>
            <input 
              type="date" 
              value={endDate}
          ...
          <div className="grid grid-cols-2 gap-3">
             ...
             <div>
               <div className="relative">
                 ...
                 <input type="text" placeholder="時間" value={flightInfo.outbound.time} onChange={e => handleOutboundChange('time', e.target.value)} className="w-full pl-9 pr-3 py-2 bg-slate-50 rounded-xl text-sm font-medium outline-none focus:ring-2 ring-indigo-100" />
               </div>
             </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
             ...
             <div>
               <div className="relative">
                 <Clock size={14} className="absolute left-3 top-3 text-slate-400" />
                 <input type="text" placeholder="時間" value={flightInfo.inbound.time} onChange={e => handleInboundChange('time', e.target.value)} className="w-full pl-9 pr-3 py-2 bg-slate-50 rounded-xl text-sm font-medium outline-none focus:ring-2 ring-orange-100" />
               </div>
             </div>
          </div>
        </div>
