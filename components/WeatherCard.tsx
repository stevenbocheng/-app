...
const WeatherIcon = ({ condition, size = 24 }: { condition: string, size?: number }) => {
  switch (condition) {
    ...
    case 'snow': return <Snowflake size={size} color="#A7F3D0" fill="#A7F3D0" fillOpacity={0.2} />;
    case 'thunder': return <CloudLightning size={size} color="#6366F1" fill="#6366F1" fillOpacity={0.2} />;
    case 'fog': return <CloudFog size={size} color="#94A3B8" fill="#94A3B8" fillOpacity={0.2} />;
        ...
        item.condition === 'sunny' ? <Sun size={24} color="white" fill="white" fillOpacity={0.3} /> :
          item.condition === 'rainy' ? <CloudRain size={24} color="white" fill="white" fillOpacity={0.3} /> :
            item.condition === 'cloudy' ? <Cloud size={24} color="white" fill="white" fillOpacity={0.3} /> :
              item.condition === 'snow' ? <Snowflake size={24} color="white" fill="white" fillOpacity={0.3} /> :
                item.condition === 'thunder' ? <CloudLightning size={24} color="white" fill="white" fillOpacity={0.3} /> :
                  item.condition === 'fog' ? <CloudFog size={24} color="white" fill="white" fillOpacity={0.3} /> :
